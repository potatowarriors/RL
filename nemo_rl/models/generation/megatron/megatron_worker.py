...
def bind_reserved_http_server_socket(port: int) -> socket.socket:
    ...
    Returns:
        The bound, listening socket, handed to the engine's `serve()` when the server starts.
    """
...
class MegatronGenerationMixin:
    ...
    def _setup_openai_api_server(self) -> str:
        ...
        if reserved_socket is not None:
            # ServeConfig at the current MLM pin has no socket handoff, so the
            # reserved socket only pins the port until just before serve():
            # close-then-rebind leaves a tiny race window (upstream sock support
            # would remove it).
            server_port = reserved_socket.getsockname()[1]
            reserved_socket.close()
            self._reserved_http_server_socket = None
        else:
    ...
    async def _generate_with_persistent_engine(
        ...
        for prompt_tokens, prompt_len, request_sampling_params in zip(
            ...
            coros.append(self.llm.generate(prompt, request_sampling_params))
